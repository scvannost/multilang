"""
This module runs Python, R, and Matlab code in the same file.
...
Expected uses are:
	1. In Python (interactive or script):
	   >>> import rython
	   >>> fname = 'path/to/file.ry'
	   >>> ry = rython.as_rython(fname)
	   >>> # or you can do
	   >>> ry = rython.Master()
...
Passing variables between environments is done using temporary .mat files.
...
# ------------------------------------ Imports ------------------------------------ #
# load everything as under so dump is easy
import numpy as np
import pandas as pd
...
# -------------------------------- Helper Functions -------------------------------- #
def py_to_r(_line, _r_object = None):
	"""
...
@_r_object is optional; a new RObject is created if None is given.
...
Returns an RObject with the variables loaded.
	...
	if not _r_object or not _r_object.isalive:
		_r_object = RObject()
	if _to_load[0] == '':
		return _r_object
...
def py_to_mat(_line, _mat_object = None):
	"""
Move the variable names specified in _line: #! matlab -> <vars>
	from python into Optional[_mat_object].
Also accepts a comma-separated str or Iterable[str]
@_mat_object is optional; a new MatlabObject is created if None is given.
...
Returns a MatlabObject with the variables loaded.
	...
	if not _mat_object or not _mat_object.isalive:
...
def r_to_py(_line, _r_object = None):
	"""
Move the variable names specified in _line: #! python -> <vars>
	from _r_object back to the Python environment.
Also accepts a comma-separated str or Iterable[str].
Updates the local rython space to include these variables; you can recover them using
	`dump()` or one of its alternatives.

Returns a dictionary of the loaded variables.
	"""
	if not _r_object:
		raise ValueError('_r_object must be given.')
	elif not _r_object.isalive:
...
def r_to_mat(_line, _r_object = None, _mat_object = None):
	"""
Move the variable names specified in _line: #! matlab -> <vars>
...
def mat_to_py(_line, _mat_object = None):
	"""
Move the variable names specified in _line: #! python -> <vars>
...
Returns a dictionary of the loaded variables.
	"""
	if not _mat_object:
		raise ValueError('_mat_object must be given.')
	elif not _mat_object.isalive:
...
def mat_to_r(_line, _mat_object = None, _r_object = None):
	"""
Move the variable names specified in _line: #! R -> <vars>
...
def dump(_file='', **kwargs):
	"""
Defaults to `dump_dict`, and so returns a dict of all local variables.
...
def dumps(**kwargs):
	"""
Returns a str version of the local variables.
...
def dump_dict(_file='', **kwargs):
	"""
Returns a dict of {name: value} for all local variables.
...
Takes @_file as a str that is the file name or a file_like with a `write`
	method and outputs all the local variables as a .mat file.
...
def dump_json(_file, **kwargs):
	"""
Takes @_file as  a str that is the file name or a file_like with a `write`
	method and outputs all the local variables as a .json file.
...
def dumps_json(_file='', **kwargs):
	"""
Returns a JSON-formatted str of {name: value} for all local variables.
...
To define you own wrapping functions, use the @rython decorator in the Python environment.
...
# -------------------------------- Main Functions -------------------------------- #
def as_rython_windows(_lines):
	"""A simple interface for rython coding on Windows.\nNot yet implemented, but will recapitulate `as_rython`."""
	raise NotImplementedError('To be used in a Windows environment')


def as_rython(_lines, _r_object = None, _mat_object = None, **kwargs):
	"""
A simple interface for rython coding.
...
"Shebangs" (i.e. #! or %!) are used as the statements to both say this is rython code
	and to switch between R and Python and Matlab.
The file/str/bytes should read as so:
	[1] #! rython [R, Python, Matlab]
	[2] # code here`
	[3] #! R/Python/Matlab -> vars
	[4] # code here
	[.] # ...
	[n] #! Python -> vars

All rython scripts start with `#! rython` then an optional language.
If no initial language is given, Python is assumed.
Scripts should end with a Python switch line if the code finishes in R to
	retrieve any variables back into the Python environment.
The suggested extension for a rython file is .ry.
...
Language names are NOT case-sensitive and depend only on the existence of 'r', 'p', or 'm'.
...
Use @rython to define a function in the rython space.
...
Any built-in Python/Rython function can also be used in this way.
	"""
	...
	# format validation
	if _lines[0][:2] not in ['#!','%!'] or 'rython' not in _lines[0].lower():
		# if not rython format, just run it as Python script
		exec('\n'.join(_lines))
	...
	for _n, _i in enumerate(_lines[1:]):
		if len(_i) > 2 and _i[:2] in ['#!', '%!']:
			...
			if not any([i in _l[0].lower() for i in 'rpm']) or len(_l) > 2:
	...
	if 'rython' in _temp or 'p' in _temp:
		_lang = 'p'
	elif 'r' in _temp:
			_lang = 'r'
	elif 'm' in _temp:
	...
	# make starting objects if not given
	if not _r_object and _lang == 'r':
		_r_object = RObject()
	if not _mat_object and _lang == 'm':
		_mat_object = MatlabObject()
	...
	while _counter < len(_lines):
		...
		# if currently in python
			...
			if _current_line[:2] in ['#!','%!']:
				...
				_counter += 1
				continue
			elif '@rython' in _current_line and re.search(r'^def\s*[a-zA-Z_]+\s*\(.*?\)\s*:$', _lines[_counter+1].strip()):
				# print('rython function')
				_end = _counter + 1
				...
				while _l and _l[:2] not in ['#!','%!'] and '@rython' not in _l and _end < len(_lines)-1:
					_end += 1
					_l = _lines[_end]
...
Unlike `as_rython()`, do not pass misformatted comments to the R (no %) or Matlab (no #) environments.

m-, mat-, and matlab-based function names are all supported.
mat is the base implementation, the others just wrap.
	"""
	def __init__(self, r = True, mat = True, load_r = False, r_object = None, mat_object = None, m = True, matlab = True):
		"""
...
For @load, see `Robject.connect`.
	...
	def who(self):
		...
		return {'mat': self.who_m, 'r': self.who_r, 'py': self.who_py}
...
Also returns as a dict
		"""
		ret = self.mat_to_py(self.who_mat)
		return ret
